...
def main():
    ...
    try:
        ...
        with tab2:
            ...
            # Kita simpan hasil training di memori Streamlit agar tidak hilang saat dropdown dipilih
            ...
            # Bungkus proses konfigurasi dan training ke dalam Form
            with st.form(key="ml_training_form"):
                st.markdown("##### ⚙️ Konfigurasi Parameter Training")
                
                col_form1, col_form2 = st.columns(2)
                with col_form1:
                    max_features_input = st.number_input("Max Features TF-IDF (Semakin kecil semakin ringan)", min_value=500, max_value=5000, value=1500)
                with col_form2:
                    test_size_input = st.slider("Test Size Ratio", min_value=0.1, max_value=0.5, value=0.2)
                
                # Tombol sakral sebagai pemicu eksekusi
                submit_button = st.form_submit_button(label="🚀 Jalankan Komparasi Model")
            
            # Eksekusi HANYA terjadi jika tombol diklik
            if submit_button:
                with st.spinner("Mengekstraksi TF-IDF dan melatih model (Proses ini diisolasi)..."):
                    ...
                    # Simpan hasil ke session_state agar permanen selama sesi berjalan
            ...
            # Tampilkan Visualisasi HANYA JIKA model_trained == True
            if st.session_state.model_trained:
                # Panggil data dari session_state
                ...
                # Detail Evaluasi per Model (Sekarang dropdown ini bisa diubah tanpa mereset training!)
                ...
                # Tampilkan pesan jika belum di klik tombol "Jalankan Komparasi Model"
                st.info("💡 Silakan klik tombol **'🚀 Jalankan Komparasi Model'** di atas untuk memulai proses Machine Learning.")
